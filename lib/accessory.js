...
Accessory.prototype.set = function(value, callback, context, displayName, iid) {
  ...
  if (this.reachable) {
    ...
    if (typeof(context) !== "undefined" && typeof(context.trigger) !== "undefined" && context.trigger.includes(platform_name)) {
      ...
      if (c == "On" && typeof this.i_value !== "undefined" && typeof this.i_value[service_name] !== "undefined" && typeof this.i_value[service_name].GPIOPin !== "undefined") {
        //Try to get info from accessory characteristics
        var gpio = Utils.tryParseJSON(this.i_value[service_name].GPIOPin); // return false if user entered a nonValid JSON string
        var inverted = typeof this.i_value[service_name].GPIOPinInverted !== "undefined" ? this.i_value[service_name].GPIOPinInverted : false; 

        if (gpio) {
          for (var i = gpio.length - 1; i >= 0; i--) {              
            if (value == false){
              inverted ? rpio.open(gpio[i], rpio.OUTPUT, rpio.HIGH) : rpio.open(gpio[i], rpio.OUTPUT, rpio.LOW);
              this.log.debug("GPIO.set '%s'", gpio[i]);
            } else {
              inverted ? rpio.open(gpio[i], rpio.OUTPUT, rpio.LOW) : rpio.open(gpio[i], rpio.OUTPUT, rpio.HIGH);
              this.log.debug("GPIO.set '%s'", gpio[i]);
            }
          } 
        }   
