...
Utils.replaceHomeKitTypes = function() {

  let filename = 'HomeKitTypes.js';
  let srcBKP = path.join(path.join(__dirname, path.normalize('../../homebridge/node_modules/hap-nodejs/lib/gen/')), filename);
  let destDirBKP = path.join(__dirname, path.normalize('../../homebridge/node_modules/hap-nodejs/lib/gen/'));

  let source = path.join(path.join(__dirname, path.normalize('../lib/gen/')), filename);
  let destination = path.join(__dirname, path.normalize('../../homebridge/node_modules/hap-nodejs/lib/gen/'));

  fs.access(destDirBKP, (err) => {
    if(err)
      fs.mkdirSync(destDirBKP);

    if (fs.existsSync(path.join(destDirBKP,    (filename + '.bkp')))) {
      console.log("HomeKitTypes.js already has a backup!")
    } else {
...
Utils.tryParseJSON = function (jsonString){
    try {
